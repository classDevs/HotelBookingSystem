<?php
	...
	if(ISSET($_POST['add_guest'])){
		...
		$query2 = $conn->query("SELECT * FROM `transaction` WHERE `checkin` = '$checkin' && `room_id` = '$_REQUEST[room_id]' && `status` = 'En Attente'") or die(mysqli_error());
		$row = $query2->num_rows;
		if($checkin < date("Y-m-d", strtotime('+8 HOURS'))){	
				echo "<script>alert('Doit être présent date')</script>";
			}else{
				if($row > 0){
					echo "<div class = 'col-md-4'>
						...
						if($guest_id = $fetch['guest_id']){
							...
							$conn->query("INSERT INTO `transaction`(guest_id, room_id, status, checkin) VALUES('$guest_id', '$room_id', 'En Attente', '$checkin')") or die(mysqli_error());
