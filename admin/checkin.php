...
<body>
	...
	<div class = "container-fluid">	
		<div class = "panel panel-default">
			<?php
				$q_p = $conn->query("SELECT COUNT(*) as total FROM `transaction` WHERE `status` = 'En Attente'") or die(mysqli_error());
				$f_p = $q_p->fetch_array();
			...
			<div class = "panel-body">
				...
				<table id = "table" class = "table table-bordered">
					...
					<tbody>
						<?php
							$query = $conn->query("SELECT * FROM `transaction` NATURAL JOIN `guest` NATURAL JOIN `room` WHERE `status` = 'Enregistrement'") or die(mysqli_query());
						...
						<tr>
							...
							<td><?php echo "".$fetch['bill'].".00  DA"?></td>
...
<script type = "text/javascript">
	function confirmationCheckin(anchor){
		var conf = confirm("Êtes-vous sûr de vouloir vérifier ?");
		if(conf){
